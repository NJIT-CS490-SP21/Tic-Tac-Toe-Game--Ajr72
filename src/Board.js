...
import{winningTiles} from "./Winner";
...
export  function Board(){
    ...
    function addUser(uid,uusername){
        ...
        if(uid< 3){ //if id counter is 1 or 2 it will add user to player list
    ...
    function setUserType(uid){
        if(uid===1){ //if id is one than it is playerX
            setType(prevType=>"PlayerX");
            setMove(prevMove=>"X");
            
        }
        else if(uid===2){ //if id is 2 then the user is player
